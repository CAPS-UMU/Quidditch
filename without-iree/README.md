...
#### Using ctest (will this work without IREE? probably not...)
...
1. I added a folder to keep common C code used by test cases (for convenience), [test_case_utils](../runtime/tests/test_case_utils). Inside there is some C code my examples rely on, and also some reference code like a python script to generate C code declarations of random input data.
2. I added two examples (holaWorld and churros) that use this custom `zizgag_dispatch` library instead of Quidditch's original dispatch library, by adding two folders to `Quidditch/runtime/tests`, and updating the corresponding `CMakeLists.txt` file. 
   ```
   runtime/tests/
   |---> holaWorld
   |     |----> holaWorld.mlir // gets compiled and linked but not used
   |     |----> main.c         // the actual code that runs on DMA + Compute Core
   |     |----> out/           // where the .o file generated by compile-for-riscv.sh gets stored
   |
   |---> churros // THIS EXAMPLE IS NOT FINISHED YET, SORRY!!	 :P
   |     |----> churros.mlir // gets compiled and linked
   |     |----> main.c       // the actual code that runs on DMA + Compute Cluster
   |     |                   // main.c calls functions defined in churros.mlir
   |     |----> out/         // where the .o file generated by compile-for-riscv.sh gets stored
   |
   |---> CMakeLists.txt      // need to modify this file for the test cases to be recognized by ninja
   ```


3. For extra reference, I also added two tiled matmul examples that appear to build correctly, but they have only been confirmed to work on spike (verilator took too much time) I couldn't get spike working on this branch because the spike repo I cloned from in my [old notes](https://github.com/EmilySillars/Quidditch-zigzag/tree/tiling/zigzag-fork#2-set-up-snitch-specific-spike) was a forked repo from Markus that has since been deleted. Ravi can probably help get spike running for snitch because I remember him saying it was not complicated.

#### Running Examples   

1. Before running any examples,
   ...
   - ~~remember to set the SPIKE environment variable to your spike `riscv-isa-sim` build directory, for example:~~ ***optional since spike not working atm***

     ```
     export SPIKE="/home/hoppip/riscv-isa-sim/build"
     ```

   - **Automate this step by running** [setup.sh](setup.sh) (modify with your own paths)

2. Build and run with `zigzag-verilator-build-and-run.sh`, which calls `compile-for-riscv.sh`:

   - `compile-for-riscv.sh`: converts a module of mlir functions (without any main function) to executable riscv .o file
   - `zigzag-verilator-build-and-run.sh`: builds the test case as part of Quidditch (mostly just including Quidditch's clone of the snitch runtime repo), linking in the `.o` file obtained by `compile-for-riscv.sh`. 

    **^^ Reminder: these scripts rely on certain environment variables/paths being available!**
...
#### a. holaWorld: Run on the DMA core + 1 compute core
...
#### b. churros: Run on the DMA core + 8 compute cores (cluster) (NOT FINISHED)

- This example deploys a *tiny* bit of work onto each of the 8 compute cores, and then prints the result of the computations from the DMA core.
- Data is transferred from L3 to L1 for the compute cores to access
- When computation is complete, the data is transferred back from L1 to L3, and then the DMA core prints it out
...
1. Inside `Quidditch` directory, do

```
source ./venv/bin/activate
```

2. Make sure to set your environment variables correctly:
...
3. After making changes to any cmake files, rerun the cmake command:
...
4. After making changes to any quidditch source files, rebuild with
