...
searchSpaceCSV="$here/$basename.csv"
# constants derived from present working directory (here)
goldenOutputDir="$here/$basename/golden-out"
finalOutputDir="$here/$basename/out"
goldenJsonOutputDir="$here/$basename/golden-tiling-schemes"
jsonOutputDir="$here/$basename/tiling-schemes"
...
# Example usage:
#. run_linear_layer.sh one-run.csv genJsons status no no
...
if [[ "$genJsonsFlag" == "genJsons" ]];
    ...
    rm -rf $jsonOutputDir
    rm -rf $goldenJsonOutputDir
    mkdir -p $jsonOutputDir
    mkdir -p $goldenJsonOutputDir
