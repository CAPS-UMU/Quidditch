...
gen_cmakelists_and_source(){
    ...
    then 
        ...
        echo "set( MYRTLE_PATH \${CMAKE_CURRENT_LIST_DIR}/../../../myrtle/myrtle/myrtle.py)" > "$out/CMakeLists.txt"
        echo "iree_turbine(SRC fakeNN.py DST \${CMAKE_CURRENT_BINARY_DIR}/fakeNN.mlirbc DTYPE \"f64\" M "$M" N "$N" K "$K")" >> "$out/CMakeLists.txt"
        echo "quidditch_module(SRC \${CMAKE_CURRENT_BINARY_DIR}/fakeNN.mlirbc DST fakeNN FLAGS --mlir-disable-threading --iree-quidditch-time-disp=fakenn"$M"x"$N"x"$K" --iree-quidditch-import-tiles=$importTiles)" >> "$out/CMakeLists.txt"
        echo "quidditch_module(SRC \${CMAKE_CURRENT_BINARY_DIR}/fakeNN.mlirbc LLVM DST fakeNN_llvm FLAGS --iree-quidditch-time-disp=fakenn"$M"x"$N"x"$K" --iree-quidditch-import-tiles=$importTiles)" >> "$out/CMakeLists.txt"
...
if [[ "$status" == "status" ]];
    then
        ...
        for ts in $(grep -oE $uniquePointRegex $searchSpaceCSV)
                ...
                cp "$fakeNNDir/CMakeLists.txt" "$fakeNNDir/$origHeader.h" "$fakeNNDir/$origScheme.json" -t $myBuildDir
                # actually build
                cd $buildDir
                cmake .. -GNinja \
