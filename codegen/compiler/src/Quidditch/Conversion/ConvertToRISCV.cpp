...
ConvertToRISCV::convertToRISCVAssemblyMyrtle(MemRefMicrokernelOp kernelOp,
  ...
  // I am going to test out splitting the xdsl into two parts *****************************************
  ...
  int ret = llvm::sys::ExecuteAndWait(
    ...
    std::nullopt, redirects);
      ...
      llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> butter =
      llvm::MemoryBuffer::getFile(stdoutFile, /*IsText=*/true);
      if(butter){
        diag.attachNote() << "stderr:\n" << butter.get()->getBuffer();
      }
  
      return diag;
    

  // for now, always return error in this version of the "convertToRiscv" *******************************
